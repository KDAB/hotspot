...
docker run hotspot-build-fedora34 bash -c "cd hotspot/build/ && ctest --output-on-failure -j \$(nproc)"
docker run hotspot-build-opensuse-tumbleweed bash -c "cd hotspot/build/ && ctest --output-on-failure -j \$(nproc)"
