...
d_demangle_package="$PWD/d-demangle/ddemangle.build.tar.bz2"
...
sudo docker build --build-arg DISABLE_RUSTC_DEMANGLE="$DISABLE_RUSTC_DEMANGLE" --build-arg DISABLE_D_DEMANGLE="$DISABLE_D_DEMANGLE" \
     -t hotspot_appimage -f scripts/Dockerfile . || exit 1
