    ...
    , m_watcher(new QFutureWatcher<ProcDataList>(this))
{
    ...
    m_multiConfig->setKConfigGroup(applicationConfig(ui->applicationName->text()));

    auto saveFunction = [this](KConfigGroup group) {
        group.writeEntry(QLatin1String("params"), ui->applicationParametersBox->text());
        group.writeEntry(QLatin1String("workingDir"), ui->workingDirectory->text());
    };

    m_multiConfig->setSaveFunction(saveFunction);
    ...
    m_multiConfig->setRestoreFunction(restoreFunction);
